...
record_agent = Agent[UserContext](
    ...
    tools=[
        retrieve_records_tool,
        schema_agent_clone.as_tool(tool_name="create_schema_tool", tool_description="A tool that create schema, WAIT FOR ITS RESPONSE before calling again, I mean do not call multiple times"),
        action_agent.as_tool(tool_name="action_record_tool", tool_description="A tool that can process job based on provided argument (action, mutli, request, schema). You prefer to call this")
    ],
